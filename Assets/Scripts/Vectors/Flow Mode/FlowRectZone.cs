...
public class FlowRectZone : FieldZone
{
    /// <summary>
    ...
    protected static readonly int
        ...
        shiftId = Shader.PropertyToID("_Shift"),
    ...
    protected void CalculatePositions() // 0 to initialize
    {
        int kernelID;
        if(numberCalculations >= 2)
        {
            kernelID = 1;
        } else
        {
        ...
        if (kernelID != 0)
        {
            positionCalculator.SetBuffer(kernelID, vectorsBufferID, field.vectorsBuffer);
            positionCalculator.SetFloat(shiftId, speed * Time.deltaTime);
        }
        ...
        positionCalculator.Dispatch(kernelID, XGroups, YGroups, ZGroups);
